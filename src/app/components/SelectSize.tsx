...
const SelectSize: FC<any> = ({ product }) => {
  const [selectedSize, setSelectedSize] = useState(product?.variant[0]?.size||'');
  const [selectedColor, setSelectedColor] = useState(product?.variant[0]?.colors[0]|| '');
  const [selectedFeature, setSelectedFeature] = useState(product?.variant[0]?.feature[0]|| '');
        ...
        <div className="text-gray-800 basis-3/4">
          <div style={{ display: "flex",flexWrap:'wrap', gap: "10px" }}>
            {product?.variant[activeVariant]?.colors?.map((color:string) => (
              ...
              >
              {color?.toLowerCase() ==='gold'&&<div className="flex gap-x-2 items-center"><span>Golden</span><FaCircle color={`${color}`} size={30}/></div>}
              {color?.toLowerCase() ==='black'&&  <div className="flex gap-x-2 items-center"><span>Black</span><FaCircle color={`${color}`} size={30}/></div>}
              {color?.toLowerCase() ==='silver'&& <div className="flex gap-x-2 items-center"><span>Silver</span> <FaCircle color={`${color}`} size={30}/></div>}
              </div>
            //   <button
            //     key={color}
            //     onClick={() => setSelectedColor(color)}
